...
public class MetaDataTest {
	 ...
	 * 获得表或视图中的所有列信息
	 */
	public void getTableColumns(String ds, String tableName) {

		try {

			ResultSet rs = dbMetaData.getColumns(null, DsUtil.getSchemaPattern(ds), tableName, "%");
			while (rs.next()) {
				...
				System.out.println(tableCat + "-" + tableSchemaName + "-" + tableName_ + "-" + columnName + "-" + dataType + "-" + dataTypeName + "-" + columnSize + "-"
...
//				System.out.println(String.format("%s - %s - %s - %s | " + dataType, columnName, dataTypeName, MetaDataType.getType(dataTypeName), JdbcType.getCode(dataType)));
//				System.out.println(String.format("put(\"%s\", %s.class);", dataTypeName, MetaDataType.getType(dataType)));
	...
	public void getIndexInfo(String schemaName, String tableName) {
		try {
			...
			while (rs.next()) {
				...
				System.out
						.println(nonUnique + "-" + indexQualifier + "-" + indexName + "-" + type + "-" + ordinalPosition + "-" + columnName + "-" + ascOrDesc + "-" + cardinality);
			}
		} catch (SQLException e) {
			e.printStackTrace();
		}
	...
	public void getAllExportedKeys(String schemaName, String tableName) {
		...
		try {
			...
			while (rs.next()) {
				...
				/**
				 * hat happens to foreign key when primary is updated: importedNoAction - do not allow update of primary key if it has been imported importedKeyCascade - change
				 * imported key to agree with primary key update importedKeySetNull - change imported key to NULL if its primary key has been updated importedKeySetDefault - change
				 * imported key to default values if its primary key has been updated importedKeyRestrict - same as importedKeyNoAction (for ODBC 2.x compatibility)
				 */
				short updateRule = rs.getShort("UPDATE_RULE");

				/**
				 * What happens to the foreign key when primary is deleted. importedKeyNoAction - do not allow delete of primary key if it has been imported importedKeyCascade -
				 * delete rows that import a deleted key importedKeySetNull - change imported key to NULL if its primary key has been deleted importedKeyRestrict - same as
				 * importedKeyNoAction (for ODBC 2.x compatibility) importedKeySetDefault - change imported key to default if its primary key has been deleted
				 */
				...
				/**
				 * can the evaluation of foreign key constraints be deferred until commit importedKeyInitiallyDeferred - see SQL92 for definition importedKeyInitiallyImmediate -
				 * see SQL92 for definition importedKeyNotDeferrable - see SQL92 for definition
				 */
				short deferRability = rs.getShort("DEFERRABILITY");

				System.out.println(pkTableCat + "-" + pkTableSchem + "-" + pkTableName + "-" + pkColumnName + "-" + fkTableCat + "-" + fkTableSchem + "-" + fkTableName + "-"
						+ fkColumnName + "-" + keySeq + "-" + updateRule + "-" + delRule + "-" + fkName + "-" + pkName + "-" + deferRability);
			}
		} catch (SQLException e) {
			e.printStackTrace();
		}
...
//		 metaData.getTableColumns(null, "test_info");
		// metaData.getIndexInfo(null, "zsc_admin");
		// metaData.getAllPrimaryKeys(null, "zsc_admin");
//		metaData.getAllExportedKeys(null, "v_test2");
