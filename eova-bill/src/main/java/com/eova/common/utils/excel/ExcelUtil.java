...
/**
 * 少于5W用Excel导出, 大于5W用Csv导出, 节省服务器资源
 *
 * @author Jieven
 *
 */
public class ExcelUtil {
	...
	private static String formatValue(MetaField f, Object value) {
		...
		case "时间框":
			if (value instanceof java.sql.Timestamp) {
				ms = ((Timestamp) value).getTime();
			} else if (value instanceof java.util.Date) {
				ms = ((Date) value).getTime();
			}

			if (ms == 0) {
				val = value.toString();
			} else {
				val = xx.time.formatLongTime(ms);
			}
			break;
		case "日期框":
			if (value instanceof java.sql.Timestamp) {
	...
	public static void main(String[] args) {
		String ts = TimestampUtil.getNow().toString();
		System.out.println(ts);
		String ds = new Date().toString();
