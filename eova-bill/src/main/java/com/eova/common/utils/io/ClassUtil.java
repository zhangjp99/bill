...
public class ClassUtil {
	...
	/**
	 * 实例化内部类(Eova内部由Mod ClassLoader 或 当前工程内的类)
	 * @param clazz
	 * @return
	 * @throws Exception
	 */
	public static <T> T newClass(String clazz) throws Exception {
		if (!xx.isEmpty(clazz)) {
			try {
				// 通过EovaModLoader进行加载, Mod没有会自动从Parent Loader进行加载
				Class<?> cs = EovaConfig.modLoader.loadClass(clazz);
				return (T) cs.newInstance();
//				Class<?> cs = Class.forName(clazz);
//				return (T) cs.newInstance();
			} catch (Exception e) {
				e.printStackTrace();
				throw new Exception("实例化类异常，请检查类是否存在:" + clazz);
			}
		}
		return null;
	}
