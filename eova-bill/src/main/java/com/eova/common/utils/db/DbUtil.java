...
public class DbUtil {
	...
	private static void appendSql(String EP, String table, String auto, StringBuilder sb, Record r) {

		sb.append("INSERT INTO " + table + " (");
		String[] names = r.getColumnNames();
		for (String n : names) {
			if (xx.isMysql() && n.equals(auto)) {
				// mysql 自增, 无需添加
				continue;
			}
	...
	public static List<String> loadSql(String sqlFilePath) {
		...
		try {
			...
			for (String sql : sqls) {
				// 屏蔽空行和注释行.replaceAll("--.*", "")
				if (xx.isEmpty(sql) || sql.startsWith("--")) {
					continue;
				}
